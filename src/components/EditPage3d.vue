  ...
  <div class="edit-page">
    ...
    <div class="side-menu side-menu-left" :class="{ open: leftMenuOpened }">
      <div class="content-container">
        ...
        <div @click="handleSaveProject" class="left-menu-item">
      ...
      >
        ...
        <div class="controls-container-right">
          <div class="controls-container-group big-icon">
            ...
            <el-button size="small" :disabled="!this.rightSprite" @click="goUp">
  ...
  methods: {
    ...
    async undo() {
      ...
      switch (currentRecord.type) {
        case "新增":
          let cur_rightImg = this.rightImages.find(
            (v) => v.name === currentRecord.name
          );
          let cur_sp = this.cv2.stage.getChildByName(currentRecord.name);
          this.recycleBin[currentRecord.name] = {
            sp: cur_sp,
            rImg: cur_rightImg,
          };
        ...
        case "移动":
          ...
          this[UPDATE_RIGHT_IMAGES]({
            ...
            data: {
              name: currentRecord.name,
              x: currentRecord.lastInfo.x,
              y: currentRecord.lastInfo.y,
            },
          });
          break;
        case "翻转":
            ...
            getInterSeg(isonserver, last, label)
              .then((data) => {
                ...
                let record = {
                  ...
                  scaleX: this.rightSprite.scale.x,
                  scaleY: this.rightSprite.scale.y,
                  leftSpriteInfo: this.rightImages.find(
                    (v) => v.name === this.rightSprite.name
                  ).leftSpriteInfo,
                  label,
                  lastInfo,
                };
    ...
    addInteractive(sp) {
      ...
      sp.on("mousedown", (e) => {
        ...
        this[UPDATE_RIGHT_IMAGES]({
          type: "select",
          data: { selectedTargetName: ct.name },
        });
        ...
        var up = (e) => {
          ...
          this[UPDATE_RIGHT_IMAGES]({
            type: "edit",
            data: { name: ct.name, x: ct.x, y: ct.y },
          });
    ...
    restore(listItem) {
      ...
      this[UPDATE_RIGHT_IMAGES]({
        type: "select",
        data: { selectedTargetName: listItem.name },
      });
    },
    /**
    ...
    removeBackgroundImg(item) {
      this.rightBackGroundImgs = this.rightBackGroundImgs.filter(
        (v) => v !== item
      );
    },
    handleWindowResize() {
      let { width, height } = getComputedStyle(this.$refs.sourceContainer);
      this.cv1.view.style.width = this.cv2.view.style.width = width;
      this.cv1.view.style.height = this.cv2.view.style.height = height;
    ...
    handleMouseWheel(e) {
      ...
      if (this.rightHoverSprite) {
        if (e.wheelDelta > 0) {
          ...
          this.rightHoverSprite.scale.x +=
            (0.02 * this.rightHoverSprite.scale.x) /
            Math.abs(this.rightHoverSprite.scale.x);
          this.rightHoverSprite.scale.y +=
            (0.02 * this.rightHoverSprite.scale.y) /
            Math.abs(this.rightHoverSprite.scale.y);
        } else {
    ...
    async handleDoneClick() {
      ...
      try {
        ...
        let lsBase64 = await this.changeLeftSprite(
          this.leftSprite,
          data.data.mask,
          info
        );
        ...
        this.$refs.drawBoard.cleanBoard();
        //保存图片列表
        let rightSpInfo = createRightImage(
          name,
          data.data.mask,
          data.data.front,
          this.rightSprite.x,
    ...
    lockSprite(itemFromRightImgs) {
      if (itemFromRightImgs.locked) {
        ...
        this.cv2.stage.getChildByName(
          itemFromRightImgs.name
        ).interactive = true;
      } else {
    ...
    scale(direction) {
      ...
      if (direction == "enlarge") {
        ...
        this.rightSprite.scale.x +=
          (0.1 * this.rightSprite.scale.x) / Math.abs(this.rightSprite.scale.x);
        this.rightSprite.scale.y +=
          (0.1 * this.rightSprite.scale.y) / Math.abs(this.rightSprite.scale.y);
    ...
    goDown() {
      ...
      let par = cur.parent;
      let zIndex = par.getChildIndex(cur);
      if (zIndex !== 1) par.setChildIndex(cur, --zIndex);
    },
    resetSprite() {
    ...
    handleDownloadFussion() {
      ...
      mask.drawRect(
        0,
        0,
        this.cv2.stage.children[0].width,
        this.cv2.stage.children[0].height
      );
      mask.endFill();
      ...
      setTimeout(() => {
        ...
        img.onload = () => {
          ...
          composePicture(base64)
            ...
            .then((res) => {
              let src = res.data.result.replace(
                "./",
                "http://10.112.224.111:8900/flaskr2/"
              );
    ...
    async handleSaveProject() {
      ...
      let result_base = saveWorkspace({ img: rightTextureDataQueue }).then(
        (data) => {
...
.picture-stack {
  ...
  .picture-stack-backgrounds-container {
    ...
    .bgstack-enter-active,
    .bgstack-leave-active {
